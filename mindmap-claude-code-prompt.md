...
The product is a mind-mapping tool, not a general whiteboard. Everything serves one loop: _think → press a key → node appears in the right place → map reflows smoothly_. The bar:
...
**Undo/redo:** `Y.UndoManager` tracking `nodes` + `links` + `meta`, `trackedOrigins = {localOrigin}`. During continuous drags, write intermediate positions with a _non-tracked_ origin and commit the final value with the tracked origin so one drag = one undo step. Same for text edits (commit on blur/Esc/Enter).
...
- Spacing tokens (user-adjustable in the style panel, stored per board): `levelGap=56`, `siblingGap=14`, `branchGap=28`, `compactness` slider scaling all three (0.6×–1.6×). _This is deliberately better than Miro, where fixed spacing is a top user complaint._
- `collapsed:true` ⇒ subtree contributes only the node's own box to packing.
- `layout:'manual'` nodes: final position = computed slot + `(mx,my)`; their _subtree_ still auto-lays-out relative to them. A root with auto-layout toggled off freezes children at current offsets (write their `mx/my`, set `layout:'manual'`).
...
- Canvas: very light warm gray `#FAFAF8` with a faint dot grid `#E4E4DE`. Chrome surfaces: white, 1px `#E8E8E3` borders, soft shadows, 10px radius. Text: `#1A1A18`; muted `#8A8A82`. Accent (selection, focus, primary buttons): a confident teal `#0D9488` — _not_ the default blue/violet.
